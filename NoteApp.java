...
// 如果方法调用是在同一行，例如 obj.method1().method2().method3()，那么会从左到右依次执行，每个方法的返回值作为下一个方法的调用对象。
// 如果方法调用是在不同行，例如 obj.method1(); obj.method2(); obj.method3(); 那么会从上到下依次执行，每个方法的调用对象都是obj。
// 如果方法调用是嵌套的，例如 obj.method1(obj.method2(obj.method3()))，那么会从最内层开始执行，每个方法的返回值作为外层方法的参数。
// 流程控制---------------------------------------------------------------------
// p 流程控制语句
